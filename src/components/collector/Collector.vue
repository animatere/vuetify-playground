    ...
    <v-card class="mb-6" elevation="10">
      ...
      <v-card-text>
        ...
        <v-row class="pt-4" dense>
          <v-col cols="6">
            <v-sheet class="text-center py-3" color="green lighten-5">
          ...
          <v-col cols="6">
            <v-sheet class="text-center py-3" color="blue lighten-5">
              <div class="text-h5 red--text">
                {{ totalCardsInSet - collectedInCurrentSet }}
              </div>
    ...
    >
      <v-col cols="12">
        ...
        <p class="text-subtitle-1">
          Try adjusting your filters or selecting a different set
        </p>
      </v-col>
    </v-row>
          ...
          <v-card-text>
            <div class="d-flex justify-space-between align-center">
              ...
              <div class="text-caption text-right">
                ...
                <div>
                  Avg {{ card.cardmarket?.prices?.trendPrice ?? "N/A" }}€
                </div>
              </div>
            </div>
            <v-chip
...
const collectedInCurrentSet = computed(
  () =>
    cards.value.filter((card) => collectedCards.value.includes(card.id)).length,
);
const totalCardsInSet = computed(() => cards.value.length);
const completionPercentage = computed(() =>
  totalCardsInSet.value > 0
    ? Math.round((collectedInCurrentSet.value / totalCardsInSet.value) * 100)
    : 0,
);
const currentSetName = computed(
  () =>
    sets.value.find((set) => set.id === selectedSet.value)?.name || "All Sets",
);
...
function applyFilters() {
  ...
  if (
    ...
    selectedRarities.value.length < availableRarities.value.length
  ) {
...
function calculateSetProgress() {
  ...
  const progress = sets.value.map((set) => {
    ...
    const collectedSetCards = setCards.filter((card) =>
      collectedCards.value.includes(card.id),
    );
    return {
      id: set.id,
      ...
      percentage:
        setCards.length > 0
          ? Math.round((collectedSetCards.length / setCards.length) * 100)
          : 0,
    };
  });
...
async function fetchCards(setId: string) {
  try {
    ...
    const rarities = [
      ...new Set(sortedCards.map((c) => c.rarity).filter(Boolean)),
    ].sort();
...
watch(completionPercentage, (newVal) => {
  ...
  const interval = setInterval(() => {
    ...
    if (
      (step > 0 && progressValue.value >= newVal) ||
      (step < 0 && progressValue.value <= newVal)
    ) {
